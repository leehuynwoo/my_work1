...
// 신상 마우스 호버효과
const targetImg=document.querySelector('.slider3 .swiper-wrapper');
const hoverCard = document.querySelector('.hover_wrap')
...
function mouseHover() {
  const target=event.target;
  const data=target.dataset.set;
  if(data==null){
    return;
  }
  const hoverTo=document.querySelector(data).addClass('onHover');
